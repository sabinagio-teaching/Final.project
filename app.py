...
with right:
    st.markdown("### Weather Mode")
st.metric(label="Weather", value=weather_mode)

st.caption(
    "Displays the current weather scenario used in the simulation. "
    "Weather influences transmission line stress, renewable generation, and overall grid stability."
)
st.markdown("### Total Demand")
st.metric(label="Demand", value=f"{total_demand:.1f} GW")

st.caption(
    "Total electricity consumption across the simulated grid, measured in gigawatts (GW). "
    "It represents the combined load of all nodes in the network."
)
...
st.markdown(f"""
    ...
    """, unsafe_allow_html=True)
st.markdown("**Renewable Output**: Amount of power generated from renewable sources such as wind and solar. It depends on both weather conditions and renewable penetration in the grid.")
...
st.markdown(f"""
    ...
    """, unsafe_allow_html=True)
st.markdown("**Conventional Output**: Electricity generated by traditional power plants, such as gas, coal, or nuclear, used to balance the grid and meet remaining demand.")
...
st.markdown(f"""
    ...
    """, unsafe_allow_html=True)
st.markdown("**Cross-border Flow**: Power exchanged between interconnected regions or countries. It shows how much electricity is imported or exported across the grid.")
